<div class="row">
    <div class="col-lg-8">
        <div class="content-card">
            <div class="card-head">
                <h5><i class="bi bi-pencil-square me-2"></i>Modifier le plat</h5>
            </div>
            <div class="card-body-custom">
                <div class="form-section-title">Informations du plat</div>
                <form method="post" action="index.php?page=plats&action=edit&id=<?= e($plat['id']) ?>">
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label">Nom <span class="text-danger">*</span></label>
                            <input type="text" name="nom" class="form-control" value="<?= e(old('nom', $plat['nom'])) ?>" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Prix <span class="text-danger">*</span></label>
                            <input type="number" name="prix" step="0.01" min="0" class="form-control" value="<?= e(old('prix', $plat['prix'])) ?>" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="categorie_id" class="form-select" required>
                            ...
                            <?php foreach ($categories as $categorie): ?>
                                <?php $selected = old('categorie_id', $plat['categorie_id']) == $categorie['id'] ? 'selected' : ''; ?>
                                <option value="<?= e($categorie['id']) ?>" <?= $selected ?>><?= e($categorie['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-4">
                        <label class="form-label">Description</label>
                        <textarea name="description" class="form-control" rows="3"><?= e(old('description', $plat['description'])) ?></textarea>
                    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" action="index.php?page=categories&action=quick_create">
                <input type="hidden" name="return_url" value="index.php?page=plats&action=edit&id=<?= e($plat['id']) ?>">
